...
export default function SystemStatusView() {
  ...
  return (
    ...
    >
      ...
      >
        <button onClick={() => setView('admin')} className="p-1.5 rounded-full hover:bg-white/10 active:bg-white/20 transition-colors">
          <ArrowLeft size={24} />
        </button>
        <div className="text-[17px] font-medium flex-grow">Статус системы</div>
      </div>

      <div className="flex-grow overflow-y-auto pt-14 p-4">
        <div className="bg-white rounded-xl p-4 shadow-sm">
          <Activity size={48} className="text-green-500 mx-auto mb-4" />
          <h2 className="text-[20px] font-medium text-center mb-2">Система работает нормально</h2>
          <p className="text-[15px] text-gray-600 text-center">
            Все службы функционируют в штатном режиме.
          </p>
        </div>
      </div>
    </motion.div>
